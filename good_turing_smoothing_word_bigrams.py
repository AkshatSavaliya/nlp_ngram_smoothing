...
def get_bigrams(lang_file):
    ...
    words[-1] = words[-1][:-1]
    words.append("</s>")
    split_index = int(len(words)*0.99)
...
def get_probabilities(test_file, bigram_counts, word_counts, thresh):
    test_data = open(test_file, 'r')
    probability_list = []
    values_vector = Counter(bigram_counts.values())
    for m in test_data:
        probability = 0.0
        data_words = m.lower()
        ...
        for j in test_bigrams:
            if j in bigram_counts:
                if bigram_counts[j] < 2:
                    probability += math.log(values_vector[1] / sum(values_vector))
                if bigram_counts[j] > thresh:
                    probability += math.log(bigram_counts[j]) / (word_counts[j[0]])
                else:
                    probability += math.log((((((bigram_counts[j] + 1) * values_vector[bigram_counts[j] + 1]) / values_vector[bigram_counts[j]]) - 
                    ((bigram_counts[j]*(thresh + 1)*values_vector[(thresh + 1)]) / values_vector[1]) /
                    (1 - ((thresh + 1) * values_vector[(thresh + 1)])) / values_vector[1])) / word_counts[j[0]])
            else:
                probability += math.log(values_vector[1] / sum(values_vector))
...
en_probs = get_probabilities(dir_path + '\\LangID.test.txt', en_bigrams, en_words, 5)
fr_probs = get_probabilities(dir_path + '\\LangID.test.txt', fr_bigrams, fr_words, 5)
gr_probs = get_probabilities(dir_path + '\\LangID.test.txt', gr_bigrams, gr_words, 5)
